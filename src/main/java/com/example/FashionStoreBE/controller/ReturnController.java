...
public class ReturnController {
    ...
    public ResponseEntity<?> getAllReturns(@RequestParam(required = false) String trangThai,
                                           @RequestParam(defaultValue = "0") int page,
                                           @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(returnService.getAllReturnRequests(trangThai, page, size));
    }
